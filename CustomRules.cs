...
namespace Fiddler
{
    public static class Handlers 
    {
        ...
        [ToolsAction("CheckVT")]
        public static void DoCallCheckVT()
        {
            DoCheckVT();
        }
        ...
        [BindUIButton("Check VT")]
        public static void DoCheckVT()
        {
            string apikey = ""; // ENTER YOUR API KEY HERE
			
            // Select all sessions and loop through each one
            ...
            for (int x = 0; x < arrSessions.Length; x++)
            {
                ...
                var currentHash = arrSessions[x].GetResponseBodyHash("sha256");
				// If the response body is empty, GetResponseBodyHash returns an empty string, in which case move on to the next session
                if (currentHash == String.Empty)
                {
                    continue;
                }
				// Build the request URL
                var url = string.Format("https://www.virustotal.com/vtapi/v2/file/report?apikey={0}&resource={1}", apikey, currentHash).Replace("-", "");
				// Make the request
				...
				// If we get an empty response from VT, an error has occured on their side, or more likely we got 403 Forbidden
                if (html == String.Empty)
                {
                    arrSessions[x].oFlags["ui-comments"] = "Error";
                    arrSessions[x].RefreshUI();
                    continue;
                }
                VTRep report = new JavaScriptSerializer().Deserialize<VTRep>(html);
				// Response code 0 usually means the hash was not found on VT. Assumptions are being made here.
                if (report.response_code == 0)
                {
                    score = "VT: N/A";
                }
				// Mark the sessions according to their detecion rate. 
				// 0 = green, benign. 
				// 1-4 = orange, could be suspicious. 
				// >4 = red, probably malicious.
                else if (report.response_code == 1)
                {
                    score = String.Format("VT: {0}/{1}", report.positives, report.total);
                    if (report.positives == 0)
                    {
                        arrSessions[x].oFlags["ui-color"] = "black";
                        arrSessions[x].oFlags["ui-backcolor"] = "lime";
                    }
					else if (report.positives <= 4)
					{
                        arrSessions[x].oFlags["ui-color"] = "black";
                ...
                arrSessions[x].oFlags["ui-comments"] = score;
                arrSessions[x].RefreshUI();
            }
